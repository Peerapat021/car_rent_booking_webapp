...
export default function TeamMembersManagementPage({ users }: { users: User[] }) {
    ...
    useEffect(() => {
        ...
        const staffs = users.filter((u) => u.user_role === 'staff').length;
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            window.location.reload(); // วิธีง่ายที่สุดเพื่อ refresh ข้อมูลหลังลบ
        } catch (err: any) {
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-sm text-gray-400">กำลังโหลดข้อมูลทีมงาน...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="max-w-[1600px] mx-auto px-6 py-8 space-y-6">
                ...
                <div className="flex items-end justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">การจัดการทีมงาน</h1>
                        <p className="text-sm text-gray-400 mt-1">จัดการข้อมูลแอดมินและสต๊าฟทั้งหมดในระบบ</p>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={() => router.push('/admin/team_members/new')}
                        ...
                        >
                            <FaPlus size={12} /> เพิ่มสมาชิกใหม่
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <StatCard
                        ...
                        icon={<FaUsersCog className="text-blue-600" size={18} />}
                        accent="bg-blue-50"
                    />
                    <StatCard
                        label="ผู้ดูแลระบบ (Admin)"
                        value={stats.admins.toLocaleString()}
                        icon={<FaUserShield className="text-rose-600" size={18} />}
                        accent="bg-rose-50"
                    />
                    <StatCard
                        label="พนักงาน (Staff)"
                        value={stats.staffs.toLocaleString()}
                        icon={<FaUserCog className="text-indigo-600" size={18} />}
                        accent="bg-indigo-50"
                    />
                </div>
                        ...
                        <table className="w-full min-w-[1100px] text-sm">
                            <thead>
                                <tr className="border-b border-gray-100">
                                    {['ID', 'ชื่อ', 'อีเมล', 'เบอร์โทร', 'บทบาท', 'สถานะ', 'สร้างเมื่อ', 'จัดการ'].map((h) => (
                                        <th
                                            key={h}
                                    ...
                                    currentItems.map((user) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-5 py-4">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${user.user_role === 'admin' ? 'bg-rose-50 text-rose-700' : 'bg-indigo-50 text-indigo-700'}`}>
                                                    <span className={`w-1.5 h-1.5 rounded-full ${user.user_role === 'admin' ? 'bg-rose-500' : 'bg-indigo-500'}`} />
                                                    {user.user_role === 'admin' ? 'Admin' : 'Staff'}
                                                </span>
                                            </td>
                                            <td className="px-5 py-4">
                                                <span
                                                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${user.user_blacklist ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}
                                                >
                                                    <span className={`w-1.5 h-1.5 rounded-full ${user.user_blacklist ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                                            ...
                                            <td className="px-5 py-4 text-center">
                                                <div className="flex items-center justify-center gap-1">
                                                    <button
                                                        onClick={() => router.push(`/admin/team_members/${user.id}`)}
                                                        title="แก้ไข"
                    ...
                    {filteredUsers.length > 0 && (
                        <div className="px-5 py-3.5 border-t border-gray-100 flex items-center justify-between gap-4 flex-wrap">
                            <p className="text-xs text-gray-400">
                                ...
                                </span> จาก <span className="font-medium text-gray-600">{filteredUsers.length.toLocaleString()}</span> คน
                            </p>
            ...
            {deleteModalOpen && userToDelete && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full">
                        <h2 className="text-2xl font-bold text-gray-900 mb-4">ยืนยันการลบสมาชิกทีม</h2>
                        <p className="text-gray-700 mb-6 leading-relaxed">
                            คุณแน่ใจหรือไม่ที่จะลบสมาชิกทีม
                            <br />
                            <span className="font-semibold text-gray-900">
                                "{userToDelete.name}" ({userToDelete.user_role.toUpperCase()} - ID: {userToDelete.id})
                            </span>
                            ?
                            <br /><br />
