...
function CartPage() {
  ...
  // แสดงเฉพาะรายการที่กำลังดำเนินการ (ไม่แสดง completed, cancelled, no_show)
  const visibleBookings = bookings.filter((b) =>
    ["pending", "pending_balance", "confirmed", "picked_up", "returned"].includes(b.status)
  );
      ...
      <div className="mt-3 space-y-4 px-3 md:px-4 pb-6">
        ...
        {visibleBookings.length === 0 && !loading && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-lg font-medium">ยังไม่มีการจองที่กำลังดำเนินการ</p>
          </div>
        )}
      </div>
    </div>
